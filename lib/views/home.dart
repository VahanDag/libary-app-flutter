...
class Home extends StatelessWidget {
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> getBooks() {
    return FirebaseFirestore.instance.collection("books").where("isPublish", isEqualTo: true).snapshots();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        floatingActionButton: FloatingActionButton(
            onPressed: () {
              Navigator.of(context).push(MaterialPageRoute(builder: (context) => const AddBooksView()));
            },
            child: const Icon(
              Icons.add,
              color: Colors.white,
                ...
                return Center(
                  child: ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      final book = Book.fromJson(allBooks?[index].data() as Map<String, dynamic>);
                      return BooksCard(
                        book: book,
                      );
                    },
